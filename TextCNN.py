...
class TextCNN(nn.Module):
    def __init__(self, opt, pre_trained_weight, n_node, n_categories):
        ...
        self.lin = nn.Linear(self.initial_feature, self.hidden_size)
        kernel_sizes=[1, 2, 3]

        self.prediction_transform = nn.Linear(self.hidden_size, self.n_categories, bias=True)
        self.conv = nn.ModuleList([nn.Conv1d(in_channels=self.initial_feature,
    ...
    def forward(self, items):
        ...
        outputs = self.lin(self.dropout(x))
        return outputs
...
def import_data(train_idx, val_idx, origin_idx, df):
    ...
    model = trans_to_cuda(TextCNN(args, pre_trained_weight, len(vocab_dic) + 1, num_categories))
...
def main():
    ...
    print("average acc, std", avg_acc, statistics.stdev(total_acc))
    avg_f1 = sum(total_f1)/len(total_f1)
    print("average f1, std", avg_f1, statistics.stdev(total_f1))
